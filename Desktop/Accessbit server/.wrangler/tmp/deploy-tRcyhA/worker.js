...
async function handleOAuthCallback(request, env) {
  ...
  try {
    ...
    const storePromises = sites.map(
      (site) => env.ACCESSIBILITY_AUTH.put(`Accessibility-Settings:${site.id}`, JSON.stringify({
        accessToken: tokenData.access_token,
        siteName: site.name || site.shortName || "Unknown Site",
        siteId: site.id,
        user: userData,
    ...
    return new Response(`<!DOCTYPE html>
        <html>
          ...
          <body>
            ...
            <p>To customize your widget, open the app from your site's Apps & Integrations panel.</p>
            <p class="redirect">Redirecting to your site...</p>
            <script>
  // No session storage for Apps & Integrations flow
  // User will authenticate when they open the app interface
  
  // Redirect back to the first site's designer
  setTimeout(() => {
...
async function handleRegisterScript(request, env) {
  try {
    ...
    const scriptUrl = "https://cdn.jsdelivr.net/gh/snm62/accessibility-test@67756f6/accessibility-widget.js";
...
async function handleApplyScript(request, env) {
  try {
    ...
    const isAccessibilityScript = /* @__PURE__ */ __name((url) => {
      return url && (url === scriptUrl || // exact match
