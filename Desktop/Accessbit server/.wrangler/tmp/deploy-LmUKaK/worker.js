...
async function handlePublishSettings(request, env) {
  ...
  try {
    ...
    const dataToStore = {
      ...
      // We'll get this from the auth data
...
async function handleRegisterScript(request, env) {
  try {
    ...
    if (existingScriptsResponse.ok) {
      ...
      const existingScript = existingScripts.scripts?.find(
        (script) => script.scriptUrl && script.scriptUrl.includes("accessibility-widget")
      );
      if (existingScript) {
    ...
    const scriptUrl = "https://cdn.jsdelivr.net/gh/snm62/snm-accessibility-widget@6cc3d38/accessibility-widget.js";
...
async function handleApplyScript(request, env) {
  try {
    ...
    const already_registered_scripts = await fetch(`https://api.webflow.com/v2/sites/${authResult.siteId}/registered_scripts/hosted `, {
      headers: {
        "Authorization": `Bearer ${authResult.accessToken}`,
        "accept-version": "2.0.0"
      }
    });
    console.log(already_registered_scripts, "already registered script");
    let existingScripts = [];
